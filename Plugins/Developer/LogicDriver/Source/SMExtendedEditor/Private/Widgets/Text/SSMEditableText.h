...
#endif
#include "Text/SMEditableTextLayout.h"
...
/**
 *
 * Reconstructed from UE. We almost got away with just overloading the MultiLineEditableText class and adding a GetTextLayout call,
 * but we needed to expand the Layout class more so we have to inject our new one in this. The layout class has very little support for derived classes.
 *
 *
*/
class SSMEditableText : public SWidget, public ISlateEditableTextWidget
{
public:
	/** Called when the cursor is moved within the text area */
	DECLARE_DELEGATE_OneParam(FOnCursorMoved, const FTextLocation&);

	SLATE_BEGIN_ARGS(SSMEditableText)
		: _Text()
		, _HintText()
		, _SearchText()
		, _Marshaller()
	...
	FSMEditableTextLayout* GetTextLayout() const { return EditableTextLayout.Get(); }
...
protected:
	...
	TUniquePtr<FSMEditableTextLayout> EditableTextLayout;

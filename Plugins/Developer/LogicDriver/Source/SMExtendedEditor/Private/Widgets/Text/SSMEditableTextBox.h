...
/**
 * 
 * This is a recreation of SMultiLineEditableTextBox. It's almost entirely the same except we inject our own SSMEditableText as the EditableText.
 * We would override the class instead if several properties and methods which we need to access in 'Construct' weren't private.
 * 
 */
class SSMEditableTextBox : public SBorder
{

public:

	SLATE_BEGIN_ARGS(SSMEditableTextBox)
	...
	SLATE_STYLE_ARGUMENT(FEditableTextBoxStyle, Style)
		...
		SLATE_EVENT(SSMEditableText::FOnCursorMoved, OnCursorMoved)
	...
	/** DIALOGUE SUITE Access to the editable text so we can access the shared layout. */
	SSMEditableText* GetEditableText() const
	{
		return EditableText.Get();
	}
...
protected:
	...
	TSharedPtr< SSMEditableText > EditableText;

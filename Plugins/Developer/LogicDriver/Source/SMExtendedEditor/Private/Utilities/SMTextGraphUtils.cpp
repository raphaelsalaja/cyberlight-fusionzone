...
void FSMTextGraphUtils::GetAllNodesWithTextPropertiesNested(UBlueprint* Blueprint, TArray<USMGraphNode_Base*>& NodesOut, const FName& ParsedNameFilter)
{
	...
	for (USMGraphNode_Base* Node : AllNodes)
	{
		...
		{
			if (USMGraphK2Node_TextPropertyNode* TextProperty = Cast<USMGraphK2Node_TextPropertyNode>(GraphPropertyNode.Value))
			{
				if (USMTextPropertyGraph* TextPropertyGraph = Cast<USMTextPropertyGraph>(TextProperty->GetPropertyGraph()))
				{
...
void FSMTextGraphUtils::RefreshTextProperties(UBlueprint* InBlueprint, const FName& ContainingParsedName)
{
	if (USMBlueprint* SMBlueprint = Cast<USMBlueprint>(InBlueprint))
	{
		...
		GetAllNodesWithTextPropertiesNested(InBlueprint, TextPropertyNodes, ContainingParsedName);
...
{
	return TextSerializer.ToTextDynamicFunctionName;
}

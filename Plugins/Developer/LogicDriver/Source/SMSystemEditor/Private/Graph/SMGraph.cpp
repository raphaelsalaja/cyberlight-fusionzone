...
const FGuid& USMGraph::GetOrGenerateTemporaryContainerGuid()
{
	if (!TemporaryContainerGuid.IsValid())
	{
		TemporaryContainerGuid = FGuid::NewGuid();
	}

	return TemporaryContainerGuid;
}

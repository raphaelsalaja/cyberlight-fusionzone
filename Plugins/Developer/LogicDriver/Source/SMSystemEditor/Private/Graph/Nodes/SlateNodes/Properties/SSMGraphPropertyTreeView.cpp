...
void SSMPropertyTreeViewRow::Construct(const FArguments& InArgs, const TSharedRef<SSMPropertyTreeView>& TreeView)
{
	Item = InArgs._Item;

	const FSuperRowType::FArguments SuperArgs;
	SMultiColumnTableRow<FPropertyTreeItemPtr>::Construct(SuperArgs, TreeView);
}
...
TSharedRef<SWidget> SSMPropertyTreeViewRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	if (ItemPtr.IsValid() && ColumnName == SSMNodeInstancePropertyView::PropertyColumnName())
	{
		...
		if (ItemPtr->PropertyWidget.IsValid())
		{
			Border->SetContent(
				...
				[
					ItemPtr->PropertyWidget.ToSharedRef()
				]);
		}
...
{
	return SNew(SSMPropertyTreeViewRow, PropertyTreeView.ToSharedRef()).Item(Item);
}
...
void SSMNodeInstancePropertyView::OnItemExpansionChanged(FPropertyTreeItemPtr TreeItem, bool bIsExpanded) const
{
	...
	if (!bIsExpanded && TreeItem->IsCategory() && TreeItem->IsDefaultCategory())
	{
...
void SSMNodeInstancePropertyView::PopulateTreeItems(const TArray<USMGraphK2Node_PropertyNode_Base*>& InGraphPropertyNodes)
{
	...
	for (USMGraphK2Node_PropertyNode_Base* PropertyNode : InGraphPropertyNodes)
	{
		...
		if (PropertyWidget.IsValid())
		{
			...
			FProperty* RealProperty = GraphProperty->MemberReference.ResolveMember<FProperty>(OwningTemplate->GetClass());
			check(RealProperty);
	...
	PropertyWidgets = UpdatedWidgets;

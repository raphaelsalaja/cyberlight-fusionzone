...
public:
	...
	/** Return module editor settings. Caches it for faster retrieval. */
	static const USMEditorSettings* GetEditorSettings();

	/** Return module editor settings for the project. Caches it for faster retrieval. */
	...
	static void GetAllNodeSubClasses(UClass* TargetClass, TArray<UClass*>& OutClasses);
	/** Get native and blueprint classes. */
	static void GetAllSubClasses(UClass* TargetClass, TArray<UClass*>& OutClasses, TSubclassOf<UBlueprint> TargetBlueprintClass = nullptr);
	/** Searches registry. */
	static void GetAllBlueprintSubClasses(UClass* TargetClass, TArray<UClass*>& OutClasses, TSubclassOf<UBlueprint> TargetBlueprintClass = nullptr);
	...
	/** Calls ResetCachedValues of all SM K2 graphs. */
	static void InvalidateCaches(UBlueprint* InBlueprint);
	...
	static T* ConvertNodeTo(USMGraphNode_Base* OriginalNode, bool bDontOverrideDefaultClass = false)
	{
		...
		AddNodeAction.NodeTemplate = NewObject<T>();

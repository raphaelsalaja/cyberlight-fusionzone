...
FOnBlueprintConditionallyCompiled FSMBlueprintEditorUtils::OnBlueprintPreConditionallyCompiledEvent;
FOnBlueprintConditionallyCompiled FSMBlueprintEditorUtils::OnBlueprintPostConditionallyCompiledEvent;
...
void FSMBlueprintEditorUtils::ConditionallyCompileBlueprint(UBlueprint* Blueprint, bool bUpdateDependencies, bool bRecreateGraphProperties)
{
	...
	if (!Blueprint->bBeingCompiled && !Blueprint->bQueuedForCompilation &&
		!FSMEditorConstructionManager::GetInstance()->IsRunningConstructionScripts(SMBlueprint))
	{
		if (SMBlueprint && SMBlueprint->bPreventConditionalCompile)
		{
			return;
		}

		OnBlueprintPreConditionallyCompiledEvent.Broadcast(Blueprint, bUpdateDependencies, bRecreateGraphProperties);
		...
		OnBlueprintPostConditionallyCompiledEvent.Broadcast(Blueprint, bUpdateDependencies, bRecreateGraphProperties);
	}
}
...
bool FSMBlueprintEditorUtils::DoesGraphHaveInputEvents(const UEdGraph* InGraph)
{
	...
	{
		TArray<UK2Node_InputAction*> InputEvents;
		GetAllNodesOfClassNested<UK2Node_InputAction>(InGraph, InputEvents);
	...
	{
		TArray<UK2Node_InputKey*> InputEvents;
		GetAllNodesOfClassNested<UK2Node_InputKey>(InGraph, InputEvents);
		if (InputEvents.Num() > 0)
		{
			return true;
		}
...
USMGraphK2Node_RuntimeNodeContainer* FSMBlueprintEditorUtils::GetRuntimeContainerFromGraph(UEdGraph* Graph)
{
...
void FSMBlueprintEditorUtils::InvalidateCaches(UBlueprint* InBlueprint)
{
	...
	if (InBlueprint)
	{
		if (USMGraph* SMGraph = GetRootStateMachineGraph(InBlueprint))
		{
...
void FSMBlueprintEditorUtils::FixUpAutoGeneratedFunctions(UBlueprint* Blueprint, bool bFocusTab, FCompilerResultsLog* MessageLog)
{
	...
	for (USMGraphK2* SMGraphK2 : GraphsToFix)
	{
		...
		FString TemporaryGraphName = OriginalGraphName + "_" + FGuid::NewGuid().ToString();
...
void FSMBlueprintEditorUtils::CleanupInvalidPropertyGraphs(UBlueprint* InBlueprint, FCompilerResultsLog* MessageLog)
{
	...
	for (TTuple<UEdGraph*, USMPropertyGraph*>& GraphTuple : OutGraphs)
	{
		USMPropertyGraph* PropertyGraph = GraphTuple.Value;
		if (!PropertyGraph || PropertyGraph->HasAnyFlags(RF_BeginDestroyed))
		{
			continue;
		}
		if (FBlueprintEditorUtils::FindBlueprintForGraph(PropertyGraph) == nullptr)
		{
			UEdGraph* ActualParentGraph = GraphTuple.Key;
			
			if (MessageLog)
			{
				MessageLog->Warning(TEXT("Removing property graph @@ because it is missing a blueprint owner."), PropertyGraph);
			}
			else
			{
				LDEDITOR_LOG_WARNING(TEXT("Removing property graph %s in blueprint %s because it is missing a blueprint owner."), *PropertyGraph->GetName(), *InBlueprint->GetName());
			}

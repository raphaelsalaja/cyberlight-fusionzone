...
void USMStateMachineComponent::ServerStart()
{
	...
	if (bHasAuth || !IsSimulatedProxy())
	{
		const bool bRunLocal = HasAuthorityToChangeStatesLocally() && !IsServerAndShouldWaitForOwningClient() &&
		...
		else if (bHasAuth && bRunLocal && !bUserManuallyLoadedNewStates)
		{
			SERVER_RequestFullSync();
		}

		// User has called LoadFromState
		if (bUserManuallyLoadedNewStates)
		{
...
{
	return IsClientAndCanLocallyChangeStates() || IsServerAndCanLocallyChangeStates();
}
				...
				if (FSMTransition* Transition = TransitionMap.FindRef(NetworkedTransaction.BaseGuid))
				{
					...
					if (!FromState->IsActive())
					{
						...
						else
						{
							...
							{
								LD_LOG_ERROR(TEXT("Possible Transition Desync: Previous state is not active. Previous State: %s. Next State: %s. Has valid remote role: %d.\n\
...
void USMStateMachineComponent::ConfigureInstanceNetworkSettings()
{
	...
	// State Domain
	{
		if (!HasAuthorityToExecuteLogic())
		{
			R_Instance->SetAllowStateLogic(false);
		}
	}
...
void USMStateMachineComponent::SERVER_RequestFullSync_Implementation()
{
	FSMFullSyncTransaction FullSyncTransaction;
	if (PrepareFullSyncTransaction(FullSyncTransaction))
	{
...
{
	if (!FullSyncTransaction.bOriginatedFromServer && IsServerAndNeedsOwningClientSync())
	{
	...
	const FSMFullSyncTransaction& FullSyncTransaction)
{
	...
	if (!HasAuthority() &&
		...
		(!bClientInSync || !HasAuthorityToChangeStates() || FullSyncTransaction.bFromUserLoad))
	{

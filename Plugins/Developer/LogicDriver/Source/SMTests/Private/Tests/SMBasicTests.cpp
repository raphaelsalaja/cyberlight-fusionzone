...
static void TestConduit(FAutomationTestBase* Test, USMOrderTransition* IncomingTransition, USMOrderConduit* Conduit, USMOrderTransition* OutgoingTransition)
{
	...
	Test->TestEqual("Conduit methods hit", Conduit->ConduitEnteredEventHit.Count, 1);
	...
	if (IncomingTransition)
	{
		...
		Test->TestEqual("Transition methods hit", IncomingTransition->TransitionEnteredEventHit.Count, 1);
	...
	if (OutgoingTransition)
	{
		...
		Test->TestEqual("Transition methods hit", OutgoingTransition->TransitionEnteredEventHit.Count, 1);
...
static void TestStandardOrder(FAutomationTestBase* Test, USMStateInstance_Base* State, USMOrderTransition* OutgoingTransition)
{
	check(State);

	if (USMStateTestInstance* StateTest = Cast<USMStateTestInstance>(State))
	{
		Test->TestEqual("State methods hit", StateTest->StateBeginHit.Count, 1);
		Test->TestEqual("State methods hit", StateTest->StateEndHit.Count, 1);
		Test->TestEqual("State methods hit", StateTest->StateInitializedEventHit.Count, 1);
		Test->TestEqual("State methods hit", StateTest->StateShutdownEventHit.Count, 1);
	...
	else if (USMOrderStateMachine* OrderStateMachine = Cast<USMOrderStateMachine>(State))
	{
		...
		Test->TestTrue("State machine ran in correct order", OrderStateMachine->Time_End < OrderStateMachine->Time_Shutdown);
		Test->TestTrue("State machine ran in correct order", OrderStateMachine->Time_Shutdown < OrderStateMachine->Time_OnCompleted);
	...
	else if (USMOrderConduit* OrderConduit = Cast<USMOrderConduit>(State))
	{
		...
		TestConduit(Test, PreviousTransition, OrderConduit, NextTransition);
	}
}

static void TestAllStates(FAutomationTestBase* Test, USMStateInstance_Base* StateToTest)
{
	...
	if (StateToTest->GetOutgoingTransitions(TransitionsToTest))
	{
		...
		{
			TestStandardOrder(Test, StateToTest, CastChecked<USMOrderTransition>(TransitionToTest));
			TestAllStates(Test, TransitionToTest->GetNextStateInstance());
		}
	}
	else
	{
		TestStandardOrder(Test, StateToTest, nullptr);
...
static void RunOrderTest(FAutomationTestBase* Test, USMBlueprint* NewBP)
{
	...
	TestAllStates(Test, TestMachine);
}
...
static void RunCompleteOrderTest(FAutomationTestBase* Test, USMBlueprint* NewBP)
{
	...
	// Collapse nodes to sub state machine, duplicate original nodes for top level.
	{
		...
		check(CollapsedOriginalStateMachineNode && CollapsedOriginalStateMachineNode->GetStateMachineGraph()->GetEntryNode()->GetOutputNode());

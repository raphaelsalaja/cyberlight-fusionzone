...
struct FObjectComponentAndNameAsStringProxyArchive : public FObjectAndNameAsStringProxyArchive
{
	...
	virtual FArchive& operator<<(UObject*& Obj) override
	{
		...
		if (IsSaving())
		{
			UActorComponent* ActorComponent = Cast<UActorComponent>(Obj);
			if (ActorComponent)
			{
				bIsComponent = 1;
			}
...
void USMPreviewObject::GetAllActorReferences(UObject* InObject, TMap<FName, FName>& PropertyNameValue) const
{
	...
	TArray<USMUtils::FSMPropertyRetrieval> OutProperties;
	USMUtils::GetAllObjectProperties(InObject, InObject->GetClass(), OutProperties);

	for (const USMUtils::FSMPropertyRetrieval& PropertyRetrieved : OutProperties)
	{
...
void USMPreviewObject::RestoreActorReferences(UObject* InObject, ULevel* InLevel, const TMap<FName, FName>& PropertyNameValue)
{
	...
	for (USMUtils::FSMPropertyRetrieval& PropertyRetrieved : OutProperties)
	{
